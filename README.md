# Skills by Andika Andriana

A collection of [Claude Code](https://claude.ai/code) skills — automating setup, configuration, and development workflows.

## Available Skills

| Skill | Description | Version |
|-------|-------------|---------|
| [flutter-setup](https://skills.sh/andika-andriana/skills/flutter-setup) | Complete Flutter environment setup from scratch for iOS and Android — great for beginners. Installs all required tools: Homebrew, Xcode, Android Studio, Android SDK, FVM, and Flutter. | 2.0.1 |

## Usage

Install a skill via terminal:

```bash
claude skill install andika-andriana/skills/flutter-setup
```

Or install all skills from this repo:

```bash
claude skill install andika-andriana/skills
```
